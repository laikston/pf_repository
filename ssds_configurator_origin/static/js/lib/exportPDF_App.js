var pdfdoc = {y : 30, spaceY : 10, x : 15, w : 180, h : 0},
	wall = {width : 886, height : $("#blueprint_scenario_background").height()},
	
	ratio = pdfdoc.w / wall.width,
	$modeltag = $('#model-tag'),
	currentMode = 'blueprint';

pdfdoc.wallT = 95 * ratio;
...
function exportToPDF(){
	...
	setTimeout(function(){
		...
		setTimeout(function(){
	        ...
	        html2canvas(document.getElementById('insert_title'), {
	            onrendered: function(canvas) {
	                ...
	                doc.addImage(pdfTitle, 'JPEG', -2, -1, 212, ($("#insert_title").outerHeight(true)) * ratio);
	                $('#insert_title').text('');
		...
		setTimeout(function(){		
			doc.addImage(logo, 'JPEG', 160, 10, 45, 17);
			
			doc.setFont("helvetica");
			doc.setFontType("bold");
			doc.setTextColor(255);
			doc.setFontSize(23);
			//doc.text(15, 12, pdfTitle);
...
//		console.log(currentMode);
		...
		html2canvas(document.getElementById('blueprint_scenario_background'), {
	            onrendered: function(canvas) {
	            	
					renderImage = canvas.toDataURL('image/jpeg');
					doc.addImage(renderImage, 'JPEG', x, 40, w, h);
					//getModelTxt(doc, modelCode, false);
	            }
		});
			
		setTimeout(function(){

			html2canvas(document.getElementById('blueprint_scenario_background'), {//blueprint_scenario_background
					onrendered: function(canvas) {
...
function getNumberTxt(doc){
	var font = 'helvetica', 
		weight = 'normal',
		size = 6.5,
		tmpTop = pdfdoc.y + pdfdoc.spaceY,
		tmpLeft = pdfdoc.x + (96 * ratio),
		
		$h_space_top = $('.measure_height_space_top'),
		$h_space_bottom = $('.measure_height_space_bottom'),
		$h = $('#measure_height'),
		
		h_space_top = tmpTop + ($h_space_top.height() / 2 * ratio) - ($h_space_top.children('div').height() / 2 * ratio),
		h = tmpTop + ($h_space_top.height() * ratio) + ($h.height() / 2 * ratio),
		h_space_bottom = tmpTop + ($h_space_top.height() * ratio) + ($h.height() * ratio) + ($h_space_bottom.height() / 2 * ratio) - ($h_space_bottom.children('div').height() / 2 * ratio),
		...
		w = tmpLeft + ($w_space_left.width() + ($w.width() / 2) - ($w.children('div').width() / 2)) * ratio,
		w_space_right = tmpLeft + (($w_space_left.width() + $w.width() + ($w_space_right.width() / 2)) - ($w_space_right.children('div').width() / 2)) * ratio,
		
		color = {r : 255, g : 255, b : 255},
		width = {startX : [w_space_left, w, w_space_right], startY : ((pdfdoc.y + pdfdoc.spaceY) - (95 / 2 * ratio)) + 1},
		height = {startX : 182.5, startY : [h_space_top, h, h_space_bottom]};
...
function getSpecificationsData(doc, modelCode){
    ...
    doc.setTextColor(71,116,207);
	doc.text(112, 106, '* Specifications are subject to change without notice.');
