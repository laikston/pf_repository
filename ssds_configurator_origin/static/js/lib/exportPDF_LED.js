...
drawContents = (function(){
	var diagram = (function(){
		...
		function drawPowerFlow(_doc, _count, _startPos, _size, _voltage){
			...
			function drawLegend(_pos){
		        ...
		        pdf.setLineWidth(1);
		    	pdf.line(pos.x + 0.5, pos.y + 2 + disclaimerH, pos.x + 5.4, pos.y + 2 + disclaimerH);
		    	pdf.text(pos.x + 9, pos.y + 3 + disclaimerH, 'Power Interconnect Cable');
		    	
		    	pdf.setFillColor(color[1].r, color[1].g, color[1].b);
		    	pdf.rect(pos.x, pos.y + 2 + 4.5 + disclaimerH, 5.9, 3.6, 'F');
		    	pdf.text(pos.x + 9, pos.y + 3 + 6.5 + disclaimerH, 'Power Cable Input/Output');
		    		    	
		    	pdf.setFillColor(color[0].r, color[0].g, color[0].b);
		    	pdf.rect(pos.x, pos.y + 2 + 5 + 6 + disclaimerH, 5.9, 3.6, 'F');
		    	pdf.text(pos.x + 9, pos.y + 3 + 13 + disclaimerH, 'Primary Cable Routing End Point');
		    	
		    	pdf.addImage(img.url, 'JPEG', pos.x + 1 , pos.y + 2 + 5 + 6 + 7 + disclaimerH, img.size.width, img.size.height);
		    	pdf.text(pos.x + 9, pos.y + 3 + 19.5 + disclaimerH, 'Primary Power Cable Input');
			}
		};
		return {
	...
	function ledRendering(_pageNum, _completeFunc){
		var pdf = doc.pdf,
			...
			num, infoHeight, ratio, activeMode, inactiveMode, unitProp, docProp, wallProp, containerProp, displayProp, modelProp, modeltagProp, headerProp, areaProp, titleProp;
		
		unitProp = {normal: scope.unit};
		unitProp.short = (unitProp.normal == 'meter') ? 'm' : 'ft'; 
		num = setNum(unitProp.normal);
		docProp = {startX : content.startX , startY : content.startY, width : content.width, padding : (parseInt($blueprint.css('padding-top')) > 0) ? parseInt($blueprint.css('padding-top')) * ratio : 0};
		areaProp = {
		...
		displayProp = {
			col : model.display.col,
			row : model.display.row,
		...
		function drawRenderedBlueprint(){
		    ...
		    // html2canvas maxSize :: https://github.com/niklasvh/html2canvas/issues/1564
		    // html2canvas maxSize :: http://html2canvas.hertzen.com/faq/
            if(docProp.width / (areaProp.height * ratio) <= 0.05)	console.log('ie error MaxSize image ratio :: ', docProp.width / (areaProp.height * ratio)); // 0.04914004914004914
		    html2canvas($pdfSection2, {
		        onrendered: function(canvas) {
		        	var tmpCanvas, bluePrintImage, idName = 'blueprint-canvas', tmpObj = {};
...
//		            $('#pdf-image-section-txt').html('<img src="' + bluePrintImage + '" />').css('display', 'block');
//		            util.setLoadingBar(false);
		            renderProp.width = tmpObj.width = docProp.width;
		            renderProp.height = tmpObj.height = areaProp.height * ratio;
		            ...
		            if(tmpObj.height >= doc.content.height - titleProp.modelPosY){
		            	...
		            	renderProp.ratio.font = renderProp.ratio.width = tmpObj.width / docProp.width;
		            	tmpObj.startX = (docProp.width - tmpObj.width) / 2;
		            	renderProp.startX = docProp.startX + tmpObj.startX;
		            }
		            
		            pdf.addImage(bluePrintImage, 'JPEG', renderProp.startX, docProp.startY + titleProp.height, tmpObj.width, tmpObj.height);
				    
		            setModelTxt(pdf, modelProp.code);
		            setMeasureTxt(pdf);                            
		    ...
		    function specPage(){
		        ...
		        completeRendering(pdf, modelProp.code);   
		    }	
		}
		function drawRenderedScenario(){
		    ...
		    html2canvas($pdfSection2, {
		        onrendered: function(canvas) {
		            ...
		            pdf.addImage(scenarioImage, 'JPEG', docProp.startX, docProp.startY + titleProp.height, docProp.width, docProp.h);
		            setModelTxt(pdf, modelProp.code);
		            setMeasureTxt(pdf);
		...
		function setModelTxt(_doc, _modelCode){
		    var s = scope, doc = _doc, modelCode = _modelCode,
		    	ratioW = ratio * renderProp.ratio.width,
		    	ratioH = ratio * renderProp.ratio.height,
		        model = {
		    		startX : (areaProp.width / 2 + (wallProp.width / 2) - parseInt($modeltag.outerWidth(true)) + 5) * ratioW + renderProp.startX, 
		    		startY : docProp.startY + titleProp.height + docProp.padding + (infoHeight * ratioH) + ((modeltagProp.height - modelProp.nameH) / 2 * ratioH) + (modelProp.nameH * ratioH), 
		    		colorR : 255, 
		    		colorG : 255, 
		    		colorB : 255
		    	},
		    	fontSize = fontProp.size.render.model * renderProp.ratio.font;
		...
		function setMeasureTxt(_doc){
		    var s = scope, doc = _doc, color = {r : 255, g : 255, b : 255},
		        ...
		        ratioW = ratio * renderProp.ratio.width,
		    	ratioH = ratio * renderProp.ratio.height,
		        fontSize = fontProp.size.render.measure * renderProp.ratio.font;
		    ...
		    tmpTop = docProp.startY + titleProp.height + docProp.padding + (infoHeight * ratioH);
		    tmpLeft = renderProp.startX + (((areaProp.width - wallProp.width) / 2) * ratioW);

		    modelY = ((modeltagProp.height - modelProp.nameH) / 2 * ratioH) + (modelProp.nameH * ratioH);    

		    h_space_margin = (parseInt($h_space_top.children('div').css('margin-top')) * ratioH);

		    h_space_top = tmpTop + h_space_margin + modelY - renderProp.ratio.font;
		    h = tmpTop + ($h_space_top.height() * ratioH) + ($h.children('div').position().top * ratioH) + modelY - renderProp.ratio.font;
		    h_space_bottom = tmpTop + ($h_space_top.height() * ratioH) + ($h.height() * ratioH) + h_space_margin + modelY - renderProp.ratio.font;

//		    w_space_left = ($('#space-dotted').width() < $blueprint_video.width()) ? tmpLeft + tmp_space_left * ratio + 0.5 : tmpLeft + tmp_space_left * ratio + 1;
//		    w = ($('#space-dotted').width() < $blueprint_video.width()) ? tmpLeft + ($w_space_left.width() + parseInt($w.find('.width_tag').css('margin-left'))) * ratio + (($('#space-dotted').width() - $blueprint_video.width())/2 * ratio) + 0.5 : tmpLeft + ($w_space_left.width() + parseInt($w.find('.width_tag').css('margin-left'))) * ratio + 1.5;
//		    w_space_right = ($('#space-dotted').width() < $blueprint_video.width()) ? tmpLeft + ($w.width() - ($blueprint_video.width() - $('#space-dotted').width()) + tmp_space_right) * ratio + 0.5 : tmpLeft + ($w_space_left.width() + $w.width() + tmp_space_right) * ratio + 2;
		    w_space_left = tmpLeft + tmp_space_left * ratioW + renderProp.ratio.font;
		    w = tmpLeft + ($w_space_left.width() + parseInt($w.find('.width_tag').css('margin-left'))) * ratioW + renderProp.ratio.font;
		    w_space_right = tmpLeft + ($w_space_left.width() + $w.width() + tmp_space_right) * ratioW + renderProp.ratio.font;
		    
		    width = {startX : [w_space_left, w, w_space_right], startY : (headerProp.height + (docProp.startY + titleProp.height - headerProp.height)) + (parseInt($pdfSection.css('padding-top')) + wallProp.height + $measure_width_total.children('div').height() + 25) * ratioH};
		    height = {startX : renderProp.startX + renderProp.width - (74 * ratioW), startY : [h_space_top, h, h_space_bottom]};
		...
		function completeRendering(_doc, _modelCode){
			var doc = _doc, modelCode = _modelCode;
	...
	function mechanicalType(_pageNum, _completeFunc){
		...
		function drawFrameKitTypeTable(_pos){
		    ...
		    if(isMin == true){
		    	pdf.setFontSize(8);
		    	pdf.setFontType("italic");
		    	pdf.text(pos.x + 15, (pos.y - height * typeTotal) + height * rowTotal + 3.5, '* Above framkit config does not match input screen size. Pls contact local representatives to further review optimal frame kit options.');
		    }
		}
	}
	function mechanicalDrawing(_pageNum, _completeFunc){
		...
		if(img.size.height * count.row > content.height - legendH - copyH){
			img.size.height = (content.height - legendH - copyH) / count.row;
	...
	function powerFlow110(_pageNum, _completeFunc){
		var pdf = doc.pdf,
			...
			count = {col: model.display.col, row: model.display.row}, legendH = 44, disclaimerH = 8;
		
		if(size.height * count.row > content.height - legendH){
			size.height = (content.height - legendH - disclaimerH) / count.row;
