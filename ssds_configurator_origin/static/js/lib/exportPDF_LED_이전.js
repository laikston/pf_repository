var $pdfSection = $('#pdf-section'),
	$pdfSection2 = document.getElementById('pdf-section'),
	$body = $('body'),
	
	$container = document.getElementById('blueprint_container'),
	
	$modeltag = $('#model-tag'),
	$modelname = $modeltag.children('.model-name'),
	    
	$h_space_top = $('.measure_height_space_top'),
	$h_space_bottom = $('.measure_height_space_bottom'),
	$h = $('#measure_height'),
	    
	$w_space_left = $('.measure_width_space_left'),
	$w_space_right = $('.measure_width_space_right'),
	$w = $('#measure_width'), // w = pdf 캡쳐 가로, h = pdf 캡쳐 세로, ratio = pixel과 pdf 단위의 가로비율
	
	$blueprint = $('#blueprint'),
	$blueprint_btn = $('#blueprint_btn'),
	$wall = $('#scenario_middle'),
	$blueprint_video = $('#videoContainer_blueprint'),
	$render_btn = $('#render_btn'),
		
	$measure_width_total = $('#measure_width_total'),
	$measure_height_total = $('#measure_height_total'),
	$measure_number = $('.measure-number'),
		
	$choose_size_w = $('.choose-size-w'),
	$choose_size_h = $('.choose-size-h'),
	$choose_opts = $('.choose-opts'),
	
	$led_title = $('#led-title'),
	
	btn_class = 'is-active',
	
	$loadingLayer = $('#loading-layer'),
	$loading = $loadingLayer.find('.layer .loading'),
	
	backImg = 'url("/static/images/videowall_configurator/background2.png")',
	fontProp = {
		size : {
			header : {
				stitle : 50
			},
			render : {
				model : 30,
				measure : 30
			},
			specification : {
				title : 80,
				head : 40,
				data : 40
			}
		},
		weight : {
			normal : 'normal',
			bold : 'bold'
		}	
	},
	
	scope, num, infoHeight, ratio, activeMode, inactiveMode, unitProp, docProp, wallProp, containerProp, displayProp, modelProp, modeltagProp, headerProp, areaProp, specProp;

//a4
function exportToPDF(){
	scope = angular.element(document.querySelector('[ng-app=led-configurator]')).scope();
	unitProp = {normal: scope.unit};
	unitProp.short = (unitProp.normal == 'meter') ? 'm' : 'ft'; 
	num = setNum(unitProp.normal);
	docProp = {format: 'a4', startX : 15, startY : 50, width : 180, padding : (parseInt($blueprint.css('padding-top')) > 0) ? parseInt($blueprint.css('padding-top')) * ratio : 0};
	areaProp = {
			width: $pdfSection.width(),
			height: $pdfSection.outerHeight()
	};
	infoHeight = 40;
	ratio = docProp.width / areaProp.width;
	modelProp = {
			code : scope.display.name,
			nameH : $modelname.height()
	};
	headerProp = {x: 0, y: 0, width: 210, height: 30}
	wallProp = {
		width: $wall.outerWidth(),
		height: $wall.outerHeight(true),
		bottomPadding: 50
	};	
	containerProp = {
			startX : parseFloat($container.style.left) + 163,
			startY : parseFloat($container.style.top) + 40,
			width : parseFloat($container.style.width),
			height : parseFloat($container.style.height)
	};
	displayProp = {
			col : scope.videowall.col,
			row : scope.videowall.row,
			width : scope.display.pixel_size.width,
			height : scope.display.pixel_size.height
	};
	modeltagProp = {
			width : $modeltag.width(), 
			height : $modeltag.outerHeight()	
	};
	specProp = {
			width: docProp.width / 3,
			height: 100,
			startX: docProp.startX,
			startY: docProp.startY
	};
	
	if(modelProp.code == null || modelProp.code == '') {
        alert('please choose model.');
    }else{
    	setLoadingBar(true);
    	setMode();
    	docProp.pdf = new jsPDF('p', 'mm', docProp.format);    	
    	setHeader(docProp.pdf, modelProp.code);  
    	setProcessByContent();
    	drawRenderedDisplay();    	
    }
};
function setTitle(){
	
}
function setPreview(){
	
}
function setMode(){
	if($blueprint_btn.hasClass(btn_class)){
        activeMode = "blueprint";
        inactiveMode = "scenario";
    }else{
        activeMode = "scenario";
        inactiveMode = "blueprint";
    }
}
function setProcessByContent(){
	var url;
	switch(scope.upload.current){
	    case 'image' :
	        url = $blueprint_video.css('background-image');
	        if(url != backImg){
	        	$blueprint_video.css({'background-image' : backImg});
	            setTimeout(function(){
	            	$blueprint_video.css({'background-image' : url});
	            }, 1000);
	        }
	        break;
	    case 'video' :
	    	$blueprint_video.find('video').css({display : 'none'});
	        $blueprint_video.css({'background-image' : backImg});
	        break;
	    default :
	        break;
	}
}
function drawRenderedBlueprint(){
	var resetProp = {};
	
	$choose_size_w.css({display:'none'});
	$choose_size_h.css({display:'none'});
	$choose_opts.css({display:'none'});
                        
    $led_title.css({visibility:'hidden'});
    $modelname.css({color:'#6a6a6a'});
    $measure_number.css({display:'none'});

    if(scope.wall.size.width <= num.maxE)	resetProp = changeGuyPos(activeMode, scope.wall.size.width);
    html2canvas($pdfSection2, {
        onrendered: function(canvas) {
        	var tmpCanvas, bluePrintImage, idName = 'blueprint-canvas';
        	document.body.appendChild(canvas);
        	canvas.setAttribute('id', idName);
        	tmpCanvas = setBlueprintPattern(idName);
        	document.body.removeChild(canvas);
        	
            bluePrintImage = tmpCanvas.toDataURL('image/jpeg');
            docProp.pdf.addImage(bluePrintImage, 'JPEG', docProp.startX, docProp.startY, docProp.width, areaProp.height * ratio);
            setModelTxt(docProp.pdf, modelProp.code);
            setMeasureTxt(docProp.pdf);                            
            
            setTimeout(specPage, 1200);
        }
    });    
    function specPage(){
    	if(scope.wall.size.width <= num.maxE){
            $('.prop').css({left : resetProp.guyL});
            if(parseInt($pdfSection.css('width')) != areaProp.width)	$pdfSection.css({width : resetProp.pdfW});
        }
        getSpecifications(docProp.pdf, modelProp.code);
        getSpecificationsData(docProp.pdf, modelProp.code);
        completeDoc(docProp.pdf, modelProp.code);   
    }	
}
function drawRenderedScenario(){
	var resetProp = {};
	
	$measure_width_total.removeClass('ng-hide');
	$measure_height_total.removeClass('ng-hide');

	$choose_size_w.css({display:'none'});
	$choose_size_h.css({display:'none'});
    $choose_opts.css({display:'none'});
    
    $led_title.css({visibility:'hidden'});
    $modelname.css({color:'#6a6a6a'});
    $measure_number.css({display:'none'});

    if(scope.wall.size.width <= num.maxE)	resetProp = changeGuyPos(inactiveMode, scope.wall.size.width);

    html2canvas($pdfSection2, {
        onrendered: function(canvas) {
        	var tmpCanvas, scenarioImage, idName = 'blueprint-canvas';
        	document.body.appendChild(canvas);
        	canvas.setAttribute('id', idName);
        	tmpCanvas = setBlueprintPattern(idName);
        	document.body.removeChild(canvas);
        	
        	scenarioImage = tmpCanvas.toDataURL('image/jpeg');
            docProp.pdf.addImage(scenarioImage, 'JPEG', docProp.startX, docProp.startY, docProp.width, docProp.h);
            setModelTxt(docProp.pdf, modelProp.code);
            setMeasureTxt(docProp.pdf);

            setTimeout(specPage, 1200);
        }
    });
    function specPage(){
    	if(scope.wall.size.width <= num.maxE){
            $('.prop').css({left : resetProp.guyL});
            if(parseInt($pdfSection.css('width')) != areaProp.width)	$pdfSection.css({width : resetProp.pdfW});
        }
        getSpecifications(docProp.pdf, modelProp.code);
        getSpecificationsData(docProp.pdf, modelProp.code);
        completeDoc(docProp.pdf, modelProp.code);
    }
}
function drawByMode(){
	switch(activeMode){
	    case "blueprint":
	    	drawRenderedBlueprint();                    
	        break;
	
	    case "scenario":
	    	drawRenderedScenario();
	        break;
	
	    default:
	        break;
	}
}
function drawRenderedDisplay(){	
	setTimeout(function(){
        setTimeout(drawByMode, 400);
    }, 200);
}
function setScreenSpecification(){
	
}
function setProductSpecification(){
	
}
function drawCabinet(){
	
}
function drawFrameKit(){
	
}
function drawPowerFlowDiagram_110v(){
	
}
function drawPowerFlowDiagram_220v(){
	
}
function drawDataFlowDiagram(){
	
}
function setFooter(){
	
}

function setNum(_unit){
	var num = {}, unit = _unit;
    switch(unit){
        case 'meter' :
            num.minS = 2;
            num.minE = 2.1;
            num.maxS = 2.2;
            num.maxE = 3;
            break;

        case 'feet' :
            num.minS = 6.56;
            num.minE = 6.88;
            num.maxS = 6.89;
            num.maxE = 9.85;
            break;

        default :
            break;
    }
    return num;
}
function setLoadingBar(_view){
	var view = _view;
	if(view){
		$loadingLayer.addClass(btn_class);
		ssds.common.newLoading($loading);
		$body.addClass('no-scroll');
	}else{
		$loadingLayer.removeClass(btn_class);
		ssds.common.newLoadingDone($loading.find('.newLoading'));
		$body.removeClass('no-scroll');
	}
}
function completeDoc(_doc, _modelCode){
	var doc = _doc, modelCode = _modelCode;
	setLoadingBar(false);
	doc.save("Samsung_LED_" + modelCode + ".pdf");
	setTimeout(function(){
		$choose_size_w.css({display:'block'});
    	$choose_size_h.css({display:'block'});
    	$choose_opts.css({display:'block'});

        $led_title.css({visibility:'visible'});
        $modelname.css({color:'#fff'});
        if($measure_number.eq(0).css('display') != 'block')	$measure_number.css({display:'block'});
        if(activeMode == "scenario")	 $blueprint_btn.trigger('click');
        if($blueprint_video.find('video').css('display') == 'none')	$blueprint_video.find('video').css({display : 'block'});
	}, 200);
}
function changeGuyPos(_mode, _wallW){
    var mode = _mode, wallW = _wallW, $guy = $('.prop'), guyProp = {};

    guyProp.width = $guy.width();
    guyProp.left = parseInt($guy.css('left'));
    guyProp.newLeft = guyProp.left + parseInt(guyProp.width / 3);

    $('.prop').css({left : guyProp.newLeft});

    if(wallW < num.maxS)	$pdfSection.css({'width' : 1030 + 'px'});

    return {
        guyL : guyProp.left,
        pdfW : 1020
    }
}
function setModelTxt(_doc, _modelCode){
    var s = scope, doc = _doc, modelCode = _modelCode,
        model = {
    		startX : (areaProp.width / 2 + (wallProp.width / 2) - parseInt($modeltag.outerWidth(true)) + 5) * ratio + docProp.startX, 
    		startY : docProp.startY + docProp.padding + (infoHeight * ratio) + ((modeltagProp.height - modelProp.nameH) / 2 * ratio) + (modelProp.nameH * ratio), 
    		colorR : 255, 
    		colorG : 255, 
    		colorB : 255, 
    		weight : fontProp.weight, 
    		size : fontProp.size.render.model
    	};
    
    doc.setTextColor(model.colorR, model.colorG, model.colorB);
    doc.setFontSize(model.size);
    doc.text(model.startX, model.startY, modelCode);
}
function setMeasureTxt(_doc){
    var s = scope, doc = _doc, color = {r : 255, g : 255, b : 255},
        
        tmpTop, tmpLeft, modelY,
        h_space_margin, h_space_top, h, h_space_bottom,
        tmp_space_left, tmp_space_right, w_space_left, w, w_space_right,
        wall_h,        
        width,
        height;
    
    doc.setFontType(fontProp.weight.normal);
    doc.setTextColor(color.r, color.g, color.b);
    doc.setFontSize(fontProp.size.render.measure);
    
    tmp_space_left = ($w_space_left.children('div').css('margin-left') == 'auto') ? -1000 : parseInt($w_space_left.children('div').css('margin-left'));    
    tmp_space_right = ($w_space_right.children('div').css('margin-left') == 'auto') ? -1000 : parseInt($w_space_right.children('div').css('margin-left')); 
    
    tmpTop = docProp.startY + docProp.padding + (infoHeight * ratio);
    tmpLeft = docProp.startX + (((areaProp.width - wallProp.width) / 2) * ratio);

    modelY = ((modeltagProp.height - modelProp.nameH) / 2 * ratio) + (modelProp.nameH * ratio);    

    h_space_margin = (parseInt($h_space_top.children('div').css('margin-top')) * ratio);

    h_space_top = tmpTop + h_space_margin + modelY;
    h = tmpTop + ($h_space_top.height() * ratio) + ($h.children('div').position().top * ratio) + modelY;
    h_space_bottom = tmpTop + ($h_space_top.height() * ratio) + ($h.height() * ratio) + h_space_margin + modelY;

    w_space_left = ($('#space-dotted').width() < $blueprint_video.width()) ? tmpLeft + tmp_space_left * ratio + 0.5 : tmpLeft + tmp_space_left * ratio + 2;
    w = ($('#space-dotted').width() < $blueprint_video.width()) ? tmpLeft + ($w_space_left.width() + parseInt($w.find('.width_tag').css('margin-left'))) * ratio + (($('#space-dotted').width() - $blueprint_video.width())/2 * ratio) + 1.5 : tmpLeft + ($w_space_left.width() + parseInt($w.find('.width_tag').css('margin-left'))) * ratio + 10;
    w_space_right = ($('#space-dotted').width() < $blueprint_video.width()) ? tmpLeft + ($w.width() - ($blueprint_video.width() - $('#space-dotted').width()) + tmp_space_right) * ratio + 1.5 : tmpLeft + ($w_space_left.width() + $w.width() + tmp_space_right) * ratio + 18;
    
    width = {startX : [w_space_left, w, w_space_right], startY : (headerProp.height + (docProp.startY - headerProp.height)) + (parseInt($pdfSection.css('padding-top')) + wallProp.height + $measure_width_total.children('div').height() + 25) * ratio};
    height = {startX : docProp.startX + docProp.width - (78 * ratio), startY : [h_space_top, h, h_space_bottom]};    

    doc.text(height.startX, height.startY[0], $h_space_top.find('.measure-number').text());
    doc.text(height.startX, height.startY[1], $h.find('.measure-number').text());
    doc.text(height.startX, height.startY[2], $h_space_bottom.find('.measure-number').text());
    
    doc.text(width.startX[0], width.startY, $w_space_left.find('.measure-number').text());
    doc.text(width.startX[1], width.startY, $w.find('.measure-number').text());
    doc.text(width.startX[2], width.startY, $w_space_right.find('.measure-number').text());
}
function getSpecifications(_doc, _modelCode){
    var doc = _doc, modelCode = _modelCode, tmp = [], x, imgx, tmpTitle, tmpTitleX, iRow = 1, totalRow = 10, space = {x : 100, y : 40, groupY : 20},
    	modelPng = 'data:image/jpeg;base64,';
    
    doc.addPage();

    doc.setDrawColor(0);
    doc.setFillColor(232, 240, 255);
    doc.rect(specProp.startX, specProp.startY, specProp.width, specProp.height * totalRow, 'F');
   
    doc.setDrawColor(0);
    doc.setFillColor(243,247,255);
    doc.rect(specProp.startX + specProp.width, specProp.startY, specProp.width, specProp.height * totalRow, 'F');

    doc.setDrawColor(199,210,238);
    doc.setLineWidth(1);

    doc.line(specProp.startX + specProp.width, specProp.startY, specProp.startX + specProp.width, specProp.startY + specProp.height * totalRow);
    doc.line(specProp.startX + specProp.width * 2, specProp.startY, specProp.startX + specProp.width * 2, specProp.startY + specProp.height * totalRow);
    
    for(; iRow <= totalRow; ++iRow){
    	if(iRow % 2 == 0 && Math.floor(iRow / 2) != 2 ){
    		doc.line(specProp.startX, specProp.startY + specProp.height * iRow, docProp.width, specProp.startY + specProp.height * iRow);
    	}else{
    		doc.line(specProp.startX + specProp.width, specProp.startY + specProp.height * iRow, docProp.width, specProp.startY + specProp.height * iRow);
    	}    	
    }
    
    doc.setDrawColor(0);
    doc.setFillColor(67,70,85);
    doc.rect(specProp.startX, specProp.startX, docProp.width, 170, 'F');

    doc.setFontType(fontProp.weight.bold);
    doc.setFontSize(fontProp.size.specification.title);
    doc.setTextColor(255,255,255);
    doc.text(specProp.startX + 50, specProp.startX + 120, 'Specifications');

    x = docProp.width - 400 - modelCode.length * .25;
    imgx = x - 120;
    doc.text(x, specProp.startX + 120, modelCode);
    doc.addImage(modelPng, 'JPEG', imgx, specProp.startX + 60, 100, 50);

    doc.setFontType(fontProp.weight.normal);
    doc.setTextColor(71,116,207);
    doc.setFontSize(fontProp.size.specification.head);

    doc.text(specProp.startX + space.x, specProp.startY + specProp.height + space.groupY, 'DISPLAY REQUIREMENTS');
    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 1 - space.y, $('#spec_total_title').text());
    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 2 - space.y, 'Screen Configuration (WxH)');

    doc.text(specProp.startX + space.x, specProp.startY + specProp.height * 4 + space.groupY, 'DISPLAY WALL DIMENSIONS');
    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 3 - space.y, 'Dimensions');
    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 4 - space.y, 'Display Area');
    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 5 - space.y, 'Diagonal');
    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 6 - space.y, 'Weight');

    doc.text(specProp.startX + space.x, specProp.startY + specProp.height * 7 + space.groupY, 'OPTICAL PARAMETER');
    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 7 - space.y, 'Resolution');    
    tmpTitle = $('#spec_sbox_title').text();
    tmpTitleX = (angular.element(document.getElementById('spec_container')).scope().display.spec.SERIES_NAME == 'XAF Series') ? specProp.startX + specProp.width + space.x - space.groupY : specProp.startX + specProp.width + space.x;
    doc.text(tmpTitleX, specProp.startY + specProp.height * 8 - space.y, tmpTitle);

    doc.text(specProp.startX + space.x, specProp.startY + specProp.height * 9 + space.groupY, 'POWER REQUIREMENTS');
    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 9 - space.y, 'Max');
    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 10 - space.y, 'Typical');
}

function getSpecificationsData(_doc, _modelCode){
	var doc = _doc, modelCode = _modelCode, darea = isEmpty($('#spec_darea').text()).replace(/²/gi, "2"), space = {x : 100, y : 40, groupY : 20}, startX = specProp.startX + (specProp.width * 2) + space.x,
		tmpInfoX = ($('.specifications-info').attr('data-type') == 'except-series') ? 42 : 112; 

    doc.setFontType(fontProp.weight.normal);
    doc.setTextColor(119,119,119);
    doc.setFontSize(fontProp.size.specification.data);
    
    doc.text(startX, specProp.startY + specProp.height - space.y, $('#spec_total').text());
    doc.text(startX, specProp.startY + specProp.height * 2 - space.y, $('#spec_config').text());

    doc.text(startX, specProp.startY + specProp.height * 3 - space.y, $('#spec_dimension').text());
    
    doc.text(startX, specProp.startY + specProp.height * 4 - space.y, darea);
    doc.text(startX, specProp.startY + specProp.height * 5 - space.y, $('#spec_diagonal').text());
    doc.text(startX, specProp.startY + specProp.height * 6 - space.y, $('#spec_weight').text());

    doc.text(startX, specProp.startY + specProp.height * 7 - space.y, $('#spec_resolution').text());
    doc.text(startX, specProp.startY + specProp.height * 8 - space.y, $('#spec_sbox').text());
    
    doc.text(startX, specProp.startY + specProp.height * 9 - space.y, $('#spec_max').text());
    doc.text(startX, specProp.startY + specProp.height * 10 - space.y, $('#spec_typical').text());
    
    doc.setFont("helvetica", "italic");
    doc.setTextColor(71,116,207);
	doc.text(tmpInfoX, specProp.startY + specProp.height * 11 - space.y, $('.specifications-info i').text());
}
function isEmpty(_str) {
	var str = _str;
    return (!str.trim() || typeof str == 'undefined' || str === null) ? '' : str;
}
function setHeader(_doc, _modelCode){
	var doc = _doc, modelCode = _modelCode,
		logo = "data:image/jpeg;base64,";

    doc.setDrawColor(0);
    doc.setFillColor(0, 136, 208);
    doc.rect(headerProp.x, headerProp.y, headerProp.width, headerProp.height, 'F');

    setTimeout(function(){
        $('#insert_title').css({display:'block'});
        html2canvas(document.getElementById('insert_title'), {
            onrendered: function(canvas) {
                var ratio = docProp.width / 1200;

                docProp.title = canvas.toDataURL('image/jpeg');
                doc.addImage(docProp.title, 'JPEG', -1, -3, docProp.width, ($("#insert_title").outerHeight(true)) * ratio);
                $('#insert_title').text('');
                $('#insert_title').css({display:'none'});
            }
        });

        setTimeout(function(){
            doc.rect(0, 15, 210, 10, 'F');

            doc.addImage(logo, 'JPEG', 160, 17, 45, 7);

            doc.setFontType(fontProp.weight.normal);
            doc.setFontSize(fontProp.size.header.stitle);
            doc.text(15, 22, angular.element(document.getElementById('spec_container')).scope().display.spec.CATEGORY + " Model " + modelCode);
        }, 100);
    }, 100);
}
function setBlueprintPattern(_idName){
	var canvas = document.getElementById(_idName).getContext("2d"),
		width = displayProp.width,
		height = displayProp.height,
		startX = containerProp.startX + 0.5,
		startY = containerProp.startY + 0.5,
		col = { // vertical
			startY : startY,
			endY : startY + containerProp.height - 1,
			i : 1,
			total : displayProp.col
		},
		row = { // horizon
				startX : startX,
				endX : startX + containerProp.width - 1,
				i : 1,
				total : displayProp.row
		};
	
	canvas.lineWidth = 0.5; 
	canvas.strokeStyle = "rgba(137, 137, 137, 0.5)";
	canvas.beginPath();
	
	drawColLine();
	drawRowLine();
	
	function drawColLine(){		
		for(; col.i < col.total; ++col.i){
			canvas.moveTo(startX + width * col.i, col.startY);
			canvas.lineTo(startX + width * col.i, col.endY);
			canvas.stroke();
		}
	}
	
	function drawRowLine(){
		for(; row.i < row.total; ++row.i){
			canvas.moveTo(row.startX, startY + height * row.i);
			canvas.lineTo(row.endX, startY + height * row.i);
			canvas.stroke();
		}
	}
	
	return document.getElementById(_idName);
}


// a0사이즈	
//function exportToPDF(){
//	scope = angular.element(document.querySelector('[ng-app=led-configurator]')).scope();
//	unitProp = {normal: scope.unit};
//	unitProp.short = (unitProp.normal == 'meter') ? 'm' : 'ft'; 
//	num = setNum(unitProp.normal);
//	docProp = {format: 'a0', startY : 250, startX : 80, width : 2220, padding : (parseInt($blueprint.css('padding-top')) > 0) ? parseInt($blueprint.css('padding-top')) * ratio : 0};
//	areaProp = {
//			width: $pdfSection.width(),
//			height: $pdfSection.outerHeight()
//	};
//	infoHeight = 40;
//	ratio = docProp.width / areaProp.width;
//	modelProp = {
//			code : scope.display.name,
//			nameH : $modelname.height()
//	};
//	headerProp = {x: 0, y: 0, width: 2410, height: 200}
//	wallProp = {
//		width: $wall.outerWidth(),
//		height: $wall.outerHeight(true),
//		bottomPadding: 50
//	};	
//	containerProp = {
//			startX : parseFloat($container.style.left) + 163,
//			startY : parseFloat($container.style.top) + 40,
//			width : parseFloat($container.style.width),
//			height : parseFloat($container.style.height)
//	};
//	displayProp = {
//			col : scope.videowall.col,
//			row : scope.videowall.row,
//			width : scope.display.pixel_size.width,
//			height : scope.display.pixel_size.height
//	};
//	modeltagProp = {
//			width : $modeltag.width(), 
//			height : $modeltag.outerHeight()	
//	};
//	specProp = {
//			width: docProp.width / 3,
//			height: 100,
//			startX: docProp.startX,
//			startY: docProp.startY
//	};
//	
//	if(modelProp.code == null || modelProp.code == '') {
//        alert('please choose model.');
//    }else{
//    	setLoadingBar(true);
//    	setMode();
//    	docProp.pdf = new jsPDF('p', 'pt', docProp.format);    	
//    	setHeader(docProp.pdf, modelProp.code);  
//    	setProcessByContent();
//    	drawRenderedDisplay();    	
//    }
//};
//function setTitle(){
//	
//}
//function setPreview(){
//	
//}
//function setMode(){
//	if($blueprint_btn.hasClass(btn_class)){
//        activeMode = "blueprint";
//        inactiveMode = "scenario";
//    }else{
//        activeMode = "scenario";
//        inactiveMode = "blueprint";
//    }
//}
//function setProcessByContent(){
//	var url;
//	switch(scope.upload.current){
//	    case 'image' :
//	        url = $blueprint_video.css('background-image');
//	        if(url != backImg){
//	        	$blueprint_video.css({'background-image' : backImg});
//	            setTimeout(function(){
//	            	$blueprint_video.css({'background-image' : url});
//	            }, 1000);
//	        }
//	        break;
//	    case 'video' :
//	    	$blueprint_video.find('video').css({display : 'none'});
//	        $blueprint_video.css({'background-image' : backImg});
//	        break;
//	    default :
//	        break;
//	}
//}
//function drawRenderedBlueprint(){
//	var resetProp = {};
//	
//	$choose_size_w.css({display:'none'});
//	$choose_size_h.css({display:'none'});
//	$choose_opts.css({display:'none'});
//                        
//    $led_title.css({visibility:'hidden'});
//    $modelname.css({color:'#6a6a6a'});
//    $measure_number.css({display:'none'});
//
//    if(scope.wall.size.width <= num.maxE)	resetProp = changeGuyPos(activeMode, scope.wall.size.width);
//    html2canvas($pdfSection2, {
//        onrendered: function(canvas) {
//        	var tmpCanvas, bluePrintImage, idName = 'blueprint-canvas';
//        	document.body.appendChild(canvas);
//        	canvas.setAttribute('id', idName);
//        	tmpCanvas = setBlueprintPattern(idName);
//        	document.body.removeChild(canvas);
//        	
//            bluePrintImage = tmpCanvas.toDataURL('image/jpeg');
//            docProp.pdf.addImage(bluePrintImage, 'JPEG', docProp.startX, docProp.startY, docProp.width, areaProp.height * ratio);
//            setModelTxt(docProp.pdf, modelProp.code);
//            setMeasureTxt(docProp.pdf);                            
//            
//            setTimeout(specPage, 1200);
//        }
//    });    
//    function specPage(){
//    	if(scope.wall.size.width <= num.maxE){
//            $('.prop').css({left : resetProp.guyL});
//            if(parseInt($pdfSection.css('width')) != areaProp.width)	$pdfSection.css({width : resetProp.pdfW});
//        }
//        getSpecifications(docProp.pdf, modelProp.code);
//        getSpecificationsData(docProp.pdf, modelProp.code);
//        completeDoc(docProp.pdf, modelProp.code);   
//    }	
//}
//function drawRenderedScenario(){
//	var resetProp = {};
//	
//	$measure_width_total.removeClass('ng-hide');
//	$measure_height_total.removeClass('ng-hide');
//
//	$choose_size_w.css({display:'none'});
//	$choose_size_h.css({display:'none'});
//    $choose_opts.css({display:'none'});
//    
//    $led_title.css({visibility:'hidden'});
//    $modelname.css({color:'#6a6a6a'});
//    $measure_number.css({display:'none'});
//
//    if(scope.wall.size.width <= num.maxE)	resetProp = changeGuyPos(inactiveMode, scope.wall.size.width);
//
//    html2canvas($pdfSection2, {
//        onrendered: function(canvas) {
//        	var tmpCanvas, scenarioImage, idName = 'blueprint-canvas';
//        	document.body.appendChild(canvas);
//        	canvas.setAttribute('id', idName);
//        	tmpCanvas = setBlueprintPattern(idName);
//        	document.body.removeChild(canvas);
//        	
//        	scenarioImage = tmpCanvas.toDataURL('image/jpeg');
//            docProp.pdf.addImage(scenarioImage, 'JPEG', docProp.startX, docProp.startY, docProp.width, docProp.h);
//            setModelTxt(docProp.pdf, modelProp.code);
//            setMeasureTxt(docProp.pdf);
//
//            setTimeout(specPage, 1200);
//        }
//    });
//    function specPage(){
//    	if(scope.wall.size.width <= num.maxE){
//            $('.prop').css({left : resetProp.guyL});
//            if(parseInt($pdfSection.css('width')) != areaProp.width)	$pdfSection.css({width : resetProp.pdfW});
//        }
//        getSpecifications(docProp.pdf, modelProp.code);
//        getSpecificationsData(docProp.pdf, modelProp.code);
//        completeDoc(docProp.pdf, modelProp.code);
//    }
//}
//function drawByMode(){
//	switch(activeMode){
//	    case "blueprint":
//	    	drawRenderedBlueprint();                    
//	        break;
//	
//	    case "scenario":
//	    	drawRenderedScenario();
//	        break;
//	
//	    default:
//	        break;
//	}
//}
//function drawRenderedDisplay(){	
//	setTimeout(function(){
//        setTimeout(drawByMode, 400);
//    }, 200);
//}
//function setScreenSpecification(){
//	
//}
//function setProductSpecification(){
//	
//}
//function drawCabinet(){
//	
//}
//function drawFrameKit(){
//	
//}
//function drawPowerFlowDiagram_110v(){
//	
//}
//function drawPowerFlowDiagram_220v(){
//	
//}
//function drawDataFlowDiagram(){
//	
//}
//function setFooter(){
//	
//}
//
//function setNum(_unit){
//	var num = {}, unit = _unit;
//    switch(unit){
//        case 'meter' :
//            num.minS = 2;
//            num.minE = 2.1;
//            num.maxS = 2.2;
//            num.maxE = 3;
//            break;
//
//        case 'feet' :
//            num.minS = 6.56;
//            num.minE = 6.88;
//            num.maxS = 6.89;
//            num.maxE = 9.85;
//            break;
//
//        default :
//            break;
//    }
//    return num;
//}
//function setLoadingBar(_view){
//	var view = _view;
//	if(view){
//		$loadingLayer.addClass(btn_class);
//		ssds.common.newLoading($loading);
//		$body.addClass('no-scroll');
//	}else{
//		$loadingLayer.removeClass(btn_class);
//		ssds.common.newLoadingDone($loading.find('.newLoading'));
//		$body.removeClass('no-scroll');
//	}
//}
//function completeDoc(_doc, _modelCode){
//	var doc = _doc, modelCode = _modelCode;
//	setLoadingBar(false);
//	doc.save("Samsung_LED_" + modelCode + ".pdf");
//	setTimeout(function(){
//		$choose_size_w.css({display:'block'});
//    	$choose_size_h.css({display:'block'});
//    	$choose_opts.css({display:'block'});
//
//        $led_title.css({visibility:'visible'});
//        $modelname.css({color:'#fff'});
//        if($measure_number.eq(0).css('display') != 'block')	$measure_number.css({display:'block'});
//        if(activeMode == "scenario")	 $blueprint_btn.trigger('click');
//        if($blueprint_video.find('video').css('display') == 'none')	$blueprint_video.find('video').css({display : 'block'});
//	}, 200);
//}
//function changeGuyPos(_mode, _wallW){
//    var mode = _mode, wallW = _wallW, $guy = $('.prop'), guyProp = {};
//
//    guyProp.width = $guy.width();
//    guyProp.left = parseInt($guy.css('left'));
//    guyProp.newLeft = guyProp.left + parseInt(guyProp.width / 3);
//
//    $('.prop').css({left : guyProp.newLeft});
//
//    if(wallW < num.maxS)	$pdfSection.css({'width' : 1030 + 'px'});
//
//    return {
//        guyL : guyProp.left,
//        pdfW : 1020
//    }
//}
//function setModelTxt(_doc, _modelCode){
//    var s = scope, doc = _doc, modelCode = _modelCode,
//        model = {
//    		startX : (areaProp.width / 2 + (wallProp.width / 2) - parseInt($modeltag.outerWidth(true)) + 5) * ratio + docProp.startX, 
//    		startY : docProp.startY + docProp.padding + (infoHeight * ratio) + ((modeltagProp.height - modelProp.nameH) / 2 * ratio) + (modelProp.nameH * ratio), 
//    		colorR : 255, 
//    		colorG : 255, 
//    		colorB : 255, 
//    		weight : fontProp.weight, 
//    		size : fontProp.size.render.model
//    	};
//    
//    doc.setTextColor(model.colorR, model.colorG, model.colorB);
//    doc.setFontSize(model.size);
//    doc.text(model.startX, model.startY, modelCode);
//}
//function setMeasureTxt(_doc){
//    var s = scope, doc = _doc, color = {r : 255, g : 255, b : 255},
//        
//        tmpTop, tmpLeft, modelY,
//        h_space_margin, h_space_top, h, h_space_bottom,
//        tmp_space_left, tmp_space_right, w_space_left, w, w_space_right,
//        wall_h,        
//        width,
//        height;
//    
//    doc.setFontType(fontProp.weight.normal);
//    doc.setTextColor(color.r, color.g, color.b);
//    doc.setFontSize(fontProp.size.render.measure);
//    
//    tmp_space_left = ($w_space_left.children('div').css('margin-left') == 'auto') ? -1000 : parseInt($w_space_left.children('div').css('margin-left'));    
//    tmp_space_right = ($w_space_right.children('div').css('margin-left') == 'auto') ? -1000 : parseInt($w_space_right.children('div').css('margin-left')); 
//    
//    tmpTop = docProp.startY + docProp.padding + (infoHeight * ratio);
//    tmpLeft = docProp.startX + (((areaProp.width - wallProp.width) / 2) * ratio);
//
//    modelY = ((modeltagProp.height - modelProp.nameH) / 2 * ratio) + (modelProp.nameH * ratio);    
//
//    h_space_margin = (parseInt($h_space_top.children('div').css('margin-top')) * ratio);
//
//    h_space_top = tmpTop + h_space_margin + modelY;
//    h = tmpTop + ($h_space_top.height() * ratio) + ($h.children('div').position().top * ratio) + modelY;
//    h_space_bottom = tmpTop + ($h_space_top.height() * ratio) + ($h.height() * ratio) + h_space_margin + modelY;
//
//    w_space_left = ($('#space-dotted').width() < $blueprint_video.width()) ? tmpLeft + tmp_space_left * ratio + 0.5 : tmpLeft + tmp_space_left * ratio + 2;
//    w = ($('#space-dotted').width() < $blueprint_video.width()) ? tmpLeft + ($w_space_left.width() + parseInt($w.find('.width_tag').css('margin-left'))) * ratio + (($('#space-dotted').width() - $blueprint_video.width())/2 * ratio) + 1.5 : tmpLeft + ($w_space_left.width() + parseInt($w.find('.width_tag').css('margin-left'))) * ratio + 10;
//    w_space_right = ($('#space-dotted').width() < $blueprint_video.width()) ? tmpLeft + ($w.width() - ($blueprint_video.width() - $('#space-dotted').width()) + tmp_space_right) * ratio + 1.5 : tmpLeft + ($w_space_left.width() + $w.width() + tmp_space_right) * ratio + 18;
//    
//    width = {startX : [w_space_left, w, w_space_right], startY : (headerProp.height + (docProp.startY - headerProp.height)) + (parseInt($pdfSection.css('padding-top')) + wallProp.height + $measure_width_total.children('div').height() + 25) * ratio};
//    height = {startX : docProp.startX + docProp.width - (78 * ratio), startY : [h_space_top, h, h_space_bottom]};    
//
//    doc.text(height.startX, height.startY[0], $h_space_top.find('.measure-number').text());
//    doc.text(height.startX, height.startY[1], $h.find('.measure-number').text());
//    doc.text(height.startX, height.startY[2], $h_space_bottom.find('.measure-number').text());
//    
//    doc.text(width.startX[0], width.startY, $w_space_left.find('.measure-number').text());
//    doc.text(width.startX[1], width.startY, $w.find('.measure-number').text());
//    doc.text(width.startX[2], width.startY, $w_space_right.find('.measure-number').text());
//}
//function getSpecifications(_doc, _modelCode){
//    var doc = _doc, modelCode = _modelCode, tmp = [], x, imgx, tmpTitle, tmpTitleX, iRow = 1, totalRow = 10, space = {x : 100, y : 40, groupY : 20},
//    	modelPng = 'data:image/jpeg;base64,';
//    
//    doc.addPage();
//
//    doc.setDrawColor(0);
//    doc.setFillColor(232, 240, 255);
//    doc.rect(specProp.startX, specProp.startY, specProp.width, specProp.height * totalRow, 'F');
//   
//    doc.setDrawColor(0);
//    doc.setFillColor(243,247,255);
//    doc.rect(specProp.startX + specProp.width, specProp.startY, specProp.width, specProp.height * totalRow, 'F');
//
//    doc.setDrawColor(199,210,238);
//    doc.setLineWidth(1);
//
//    doc.line(specProp.startX + specProp.width, specProp.startY, specProp.startX + specProp.width, specProp.startY + specProp.height * totalRow);
//    doc.line(specProp.startX + specProp.width * 2, specProp.startY, specProp.startX + specProp.width * 2, specProp.startY + specProp.height * totalRow);
//    
//    for(; iRow <= totalRow; ++iRow){
//    	if(iRow % 2 == 0 && Math.floor(iRow / 2) != 2 ){
//    		doc.line(specProp.startX, specProp.startY + specProp.height * iRow, docProp.width, specProp.startY + specProp.height * iRow);
//    	}else{
//    		doc.line(specProp.startX + specProp.width, specProp.startY + specProp.height * iRow, docProp.width, specProp.startY + specProp.height * iRow);
//    	}    	
//    }
//    
//    doc.setDrawColor(0);
//    doc.setFillColor(67,70,85);
//    doc.rect(specProp.startX, specProp.startX, docProp.width, 170, 'F');
//
//    doc.setFontType(fontProp.weight.bold);
//    doc.setFontSize(fontProp.size.specification.title);
//    doc.setTextColor(255,255,255);
//    doc.text(specProp.startX + 50, specProp.startX + 120, 'Specifications');
//
//    x = docProp.width - 400 - modelCode.length * .25;
//    imgx = x - 120;
//    doc.text(x, specProp.startX + 120, modelCode);
//    doc.addImage(modelPng, 'JPEG', imgx, specProp.startX + 60, 100, 50);
//
//    doc.setFontType(fontProp.weight.normal);
//    doc.setTextColor(71,116,207);
//    doc.setFontSize(fontProp.size.specification.head);
//
//    doc.text(specProp.startX + space.x, specProp.startY + specProp.height + space.groupY, 'DISPLAY REQUIREMENTS');
//    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 1 - space.y, $('#spec_total_title').text());
//    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 2 - space.y, 'Screen Configuration (WxH)');
//
//    doc.text(specProp.startX + space.x, specProp.startY + specProp.height * 4 + space.groupY, 'DISPLAY WALL DIMENSIONS');
//    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 3 - space.y, 'Dimensions');
//    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 4 - space.y, 'Display Area');
//    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 5 - space.y, 'Diagonal');
//    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 6 - space.y, 'Weight');
//
//    doc.text(specProp.startX + space.x, specProp.startY + specProp.height * 7 + space.groupY, 'OPTICAL PARAMETER');
//    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 7 - space.y, 'Resolution');    
//    tmpTitle = $('#spec_sbox_title').text();
//    tmpTitleX = (angular.element(document.getElementById('spec_container')).scope().display.spec.SERIES_NAME == 'XAF Series') ? specProp.startX + specProp.width + space.x - space.groupY : specProp.startX + specProp.width + space.x;
//    doc.text(tmpTitleX, specProp.startY + specProp.height * 8 - space.y, tmpTitle);
//
//    doc.text(specProp.startX + space.x, specProp.startY + specProp.height * 9 + space.groupY, 'POWER REQUIREMENTS');
//    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 9 - space.y, 'Max');
//    doc.text(specProp.startX + specProp.width + space.x, specProp.startY + specProp.height * 10 - space.y, 'Typical');
//}
//
//function getSpecificationsData(_doc, _modelCode){
//	var doc = _doc, modelCode = _modelCode, darea = isEmpty($('#spec_darea').text()).replace(/²/gi, "2"), space = {x : 100, y : 40, groupY : 20}, startX = specProp.startX + (specProp.width * 2) + space.x,
//		tmpInfoX = ($('.specifications-info').attr('data-type') == 'except-series') ? 42 : 112; 
//
//    doc.setFontType(fontProp.weight.normal);
//    doc.setTextColor(119,119,119);
//    doc.setFontSize(fontProp.size.specification.data);
//    
//    doc.text(startX, specProp.startY + specProp.height - space.y, $('#spec_total').text());
//    doc.text(startX, specProp.startY + specProp.height * 2 - space.y, $('#spec_config').text());
//
//    doc.text(startX, specProp.startY + specProp.height * 3 - space.y, $('#spec_dimension').text());
//    
//    doc.text(startX, specProp.startY + specProp.height * 4 - space.y, darea);
//    doc.text(startX, specProp.startY + specProp.height * 5 - space.y, $('#spec_diagonal').text());
//    doc.text(startX, specProp.startY + specProp.height * 6 - space.y, $('#spec_weight').text());
//
//    doc.text(startX, specProp.startY + specProp.height * 7 - space.y, $('#spec_resolution').text());
//    doc.text(startX, specProp.startY + specProp.height * 8 - space.y, $('#spec_sbox').text());
//    
//    doc.text(startX, specProp.startY + specProp.height * 9 - space.y, $('#spec_max').text());
//    doc.text(startX, specProp.startY + specProp.height * 10 - space.y, $('#spec_typical').text());
//    
//    doc.setFont("helvetica", "italic");
//    doc.setTextColor(71,116,207);
//	doc.text(tmpInfoX, specProp.startY + specProp.height * 11 - space.y, $('.specifications-info i').text());
//}
//function isEmpty(_str) {
//	var str = _str;
//    return (!str.trim() || typeof str == 'undefined' || str === null) ? '' : str;
//}
//function setHeader(_doc, _modelCode){
//	var doc = _doc, modelCode = _modelCode,
//		logo = "data:image/jpeg;base64,";
//
//    doc.setDrawColor(0);
//    doc.setFillColor(0, 136, 208);
//    doc.rect(headerProp.x, headerProp.y, headerProp.width, headerProp.height, 'F');
//
//    setTimeout(function(){
//        $('#insert_title').css({display:'block'});
//        html2canvas(document.getElementById('insert_title'), {
//            onrendered: function(canvas) {
//                var ratio = docProp.width / 1200;
//
//                docProp.title = canvas.toDataURL('image/jpeg');
//                doc.addImage(docProp.title, 'JPEG', -140, -80, docProp.width + 200, ($("#insert_title").outerHeight(true)) * ratio);
//                $('#insert_title').text('');
//                $('#insert_title').css({display:'none'});
//            }
//        });
//
//        setTimeout(function(){
//            doc.rect(0, 15, 210, 10, 'F');
//
//            doc.addImage(logo, 'JPEG', 1650, 45, 675, 105);
//
//            doc.setFont("helvetica");
//            doc.setFontType("bold");
//            doc.setTextColor(255);
//            doc.setFontSize(23);
//
//            doc.setFontType(fontProp.weight.normal);
//            doc.setFontSize(fontProp.size.header.stitle);
//            doc.text(50, 170, angular.element(document.getElementById('spec_container')).scope().display.spec.CATEGORY + " Model " + modelCode);
//        }, 100);
//    }, 100);
//}
//function setBlueprintPattern(_idName){
//	var canvas = document.getElementById(_idName).getContext("2d"),
//		width = displayProp.width,
//		height = displayProp.height,
//		startX = containerProp.startX + 0.5,
//		startY = containerProp.startY + 0.5,
//		col = { // vertical
//			startY : startY,
//			endY : startY + containerProp.height - 1,
//			i : 1,
//			total : displayProp.col
//		},
//		row = { // horizon
//				startX : startX,
//				endX : startX + containerProp.width - 1,
//				i : 1,
//				total : displayProp.row
//		};
//	
//	canvas.lineWidth = 0.5; 
//	canvas.strokeStyle = "rgba(137, 137, 137, 0.5)";
//	canvas.beginPath();
//	
//	drawColLine();
//	drawRowLine();
//	
//	function drawColLine(){		
//		for(; col.i < col.total; ++col.i){
//			canvas.moveTo(startX + width * col.i, col.startY);
//			canvas.lineTo(startX + width * col.i, col.endY);
//			canvas.stroke();
//		}
//	}
//	
//	function drawRowLine(){
//		for(; row.i < row.total; ++row.i){
//			canvas.moveTo(row.startX, startY + height * row.i);
//			canvas.lineTo(row.endX, startY + height * row.i);
//			canvas.stroke();
//		}
//	}
//	
//	return document.getElementById(_idName);
//}